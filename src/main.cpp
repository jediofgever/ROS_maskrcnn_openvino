...
MaskRCNNInferenceOpenvino::MaskRCNNInferenceOpenvino(ros::NodeHandle *nh_ptr_) : nh_(nh_ptr_)
{
    ...
    /** Read network model **/
    network = ie.ReadNetwork("/home/atas/clearpath_ws/src/openvino_maskrcnn_inference/maksrcnn_mo/frozen_inference_graph.xml");
    ...
    segmentd_img_pub_ = nh_->advertise<sensor_msgs::Image>("openvino_segmented_image", 1);
}
    ...
    try
    {
        ...
        for (size_t box = 0; box < BOXES; ++box)
        {
            ...
            if (prob > PROBABILITY_THRESHOLD)
            {
                ...
                std::cout << "Detected class" << class_id << " with probability " << prob << " from batch " << batch
                ...
                std::cout << "Drawed detection successfully !!";
            }
        }
